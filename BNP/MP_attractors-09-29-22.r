...
getPathToAttractor(network=net,state = tmp.state)[,c('IL12_out','IL10_out','STAT6','mir155','NFKB','VEGF_out')]

# LPS and IgG stimulus
tmp.state <- rep(0,length(net$genes)); names(tmp.state) <- net$genes
tmp.state[c('LPS_e','IC_e','IL1B_e','EGFR_e')] <- 1
