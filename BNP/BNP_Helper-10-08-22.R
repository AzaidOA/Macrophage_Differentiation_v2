...
int2binState <- function(x, node.names){
    ...
    max.bit.flag <- node.len > 32
    if(max.bit.flag) node.len <- 32
    if(is.null(dim(x))) x <- as.data.frame(x)
    state <- as.integer( intToBits(x[1,])[1:node.len] )
    names(state) <- node.names[1:node.len]

    if(max.bit.flag){
      state <- c(state,as.integer( intToBits(x[2,])[1:(length(node.names)-32)] ))
